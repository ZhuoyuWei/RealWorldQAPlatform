...
public class IndexFiles {
  ...
  /**
35   * Indexes the given file using the given writer, or if a directory is given,
...
148   */
  static void indexDocs(final IndexWriter writer, Path path,String code) throws IOException {
    if (Files.isDirectory(path)) {
      Files.walkFileTree(path, new SimpleFileVisitor<Path>() {
        @Override
        public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
          try {
            indexDoc(writer, file, attrs.lastModifiedTime().toMillis(),code);
      ...
      indexDoc(writer, path, Files.getLastModifiedTime(path).toMillis(),code);
    }
  }
